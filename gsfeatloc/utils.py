...
def perturb_SE3(T_inW_ofC: np.ndarray, 
    ...
    # assert axis_mode in ['random', 'x', 'y', 'pinerf', 'icomma'], "axis_mode must be 'random', 'x', or 'y'."
    # assert magnitude_mode in ['fixed', 'uniform', 'gaussian', 'pinerf', 'icomma'], "magnitude_mode must be 'fixed', 'uniform', or 'gaussian'."

    if axis_mode == 'pinerf' and magnitude_mode == 'pinerf':
        delta_trs = rng.uniform(-0.25, 0.25, size=3)
        delta_rot = rng.uniform(-15, 15, size=3)
        
        T_perturb_trs = np.eye(4)
        T_perturb_trs[:3, 3] = delta_trs
        
        T_perturb_rot = np.eye(4)
        T_perturb_rot[:3, :3] = rotvec_to_matrix(np.deg2rad(delta_rot[0]), np.array([1, 0, 0]))
        T_perturb_rot[:3, :3] = rotvec_to_matrix(np.deg2rad(delta_rot[1]), np.array([0, 1, 0])) @ T_perturb_rot[:3, :3]
        T_perturb_rot[:3, :3] = rotvec_to_matrix(np.deg2rad(delta_rot[2]), np.array([0, 0, 1])) @ T_perturb_rot[:3, :3]

        T_inW_ofC_trs = np.eye(4)
        T_inW_ofC_trs[:3, 3] = T_inW_ofC[:3, 3]
        
        T_inW_ofC_rot = np.eye(4)
        T_inW_ofC_rot[:3, :3] = T_inW_ofC[:3, :3]

        return T_perturb_trs @ T_inW_ofC_trs @ T_perturb_rot @ T_inW_ofC_rot
    
    elif axis_mode == 'icomma' and magnitude_mode == 'icomma':
        delta_trs = rng.uniform(-0.05, 0.05, size=3)
        delta_rot = rng.uniform(-20, 20, size=3)
        ...
        return T_perturb_trs @ T_perturb_rot @ T_inW_ofC

    elif axis_mode == 'chen-rot-trs' and magnitude_mode == 'chen-rot-trs':
        ...
        R_mat = rotvec_to_matrix(np.deg2rad(angle), axis_rot)
        T_rot[:3, :3] = R_mat
        # Create translation perturbation as SE(3) matrix
        T_trs = np.eye(4)
        T_trs[:3, 3] = rng.uniform(0, 0.2) * axis_trs

        return T_inW_ofC @ T_rot @ T_trs
    
    elif axis_mode == 'chen-trs-rot' and magnitude_mode == 'chen-trs-rot':
        # Generate random rotation axis
        axis_rot = rng.random(3)
        axis_rot /= np.linalg.norm(axis_rot)
        # Generate random translation axis
        ...
        return T_inW_ofC @ T_trs @ T_rot

    elif axis_mode == 'trs-rot-chen' and magnitude_mode == 'trs-rot-chen':
        # Generate random rotation axis
        axis_rot = rng.random(3)
        axis_rot /= np.linalg.norm(axis_rot)
        # Generate random translation axis
        ...
        return T_trs @ T_rot @ T_inW_ofC
    
    elif axis_mode == 'rot-trs-chen' and magnitude_mode == 'rot-trs-chen':
        # Generate random rotation axis
        axis_rot = rng.random(3)
        axis_rot /= np.linalg.norm(axis_rot)
        # Generate random translation axis
        ...
        return T_rot @ T_trs @ T_inW_ofC

    # else:
    #     raise ValueError("Invalid axis_mode or magnitude_mode.")
